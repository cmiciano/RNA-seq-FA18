...
### v2 fixed fold change to reflect thw FC when abf3 is present
### extracted up and down reg genes from original files diffnobatch 
### and diffno3 to subset and maintain row numbers from these files
...
#403 0.5-FC 33 1-FC 
up2v3 <-subset(diffno3, diffno3$WT.vs..abf2.Log2.Fold.Change >= 0.5
...
#604 -0.5-FC 83 1-FC
down2v3 <-subset(diffno3, diffno3$WT.vs..abf2.Log2.Fold.Change <= -0.5
                     & diffno3$WT.vs..abf2.adj..p.value <= 0.05)
...
sigall2v3<- subset(diffno3, abs(diffno3$WT.vs..abf2.Log2.Fold.Change) >= 0.5
                    & diffno3$WT.vs..abf2.adj..p.value <= 0.05)
...
#Maybe order the union genes based on the order that theyre found 
...
#Preserves row numb of diff
#diff3v3 <- subset(sigall3v3$Transcript, sigall3v3 %in% sigall3v3$Transcript)# new

#Need to keep row numbers
diff3v3 <- as.character(sigall3v3$Transcript)
diffan3v3 <- as.character(sigall3v3$Annotation.Divergence)
...
diff2v3 <- as.character(sigall2v3$Transcript)
diffan2v3 <- as.character(sigall2v3$Annotation.Divergence)
...
# ##Changed fc should be the fc in 3v3
fc43nosig <- subset(diffnobatch,
                    diffnobatch$Transcript %in% zerodiff$un2and3) #new

transandfc <- fc43nosig[, c("Transcript", "WT.vs..abf2.Log2.Fold.Change")]
justch <- as.character(zerodiff$un2and3)

spli <- sapply(justch, function(x) transandfc$WT.vs..abf2.Log2.Fold.Change[which(transandfc$Transcript == x)])
zerodiff$fc <- spli

### BEFORE I FINISH RESET THE SCIENTIFIC NOTATIONS
